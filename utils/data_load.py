...
def find_pair(noisy_file_name):
    clean_dirs = []
    for noisy_path in noisy_file_name:
        if "mixture.wav" in noisy_path:
            clean_path = noisy_path.replace("mixture.wav", "clean.wav")
            if os.path.exists(clean_path):
                clean_dirs.append(clean_path)
            else:
                print(f"Warning: No clean.wav found for {noisy_path}")
    return clean_dirs
